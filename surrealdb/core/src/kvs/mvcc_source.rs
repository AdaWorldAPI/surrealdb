...
// Sprint 0/1 scaffold (borrowed from the reverted PR #24): the trait + local
// impl land first; the consumers (kv-tikv native MVCC, and the lance timeline
// version source) wire in a later sprint. Allow dead_code until then so the
// additive module does not warn the workspace.
#![allow(dead_code)]
